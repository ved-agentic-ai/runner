    ...
    <div className="flex flex-col h-full rounded-2xl border border-slate-800 bg-slate-900/80 p-4 backdrop-blur-md shadow-xl relative">
      ...
      {showTrashModal && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-slate-950/85 backdrop-blur-md rounded-2xl p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-2xl sm:max-w-3xl rounded-2xl border border-slate-700 bg-[#0b1329] p-5 shadow-2xl space-y-4 text-left flex flex-col max-h-[90%]">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button
                ...
                className="text-slate-400 hover:text-white px-2 py-1 rounded-lg bg-slate-900 border border-slate-800 text-xs font-bold"
              >
                ✕ Close
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2.5 pr-1 min-h-[220px]">
              {trashItems.length === 0 ? (
                ...
                trashItems.map((item) => (
                  <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 hover:bg-slate-900 transition-colors animate-in fade-in slide-in-from-bottom-2 duration-200">
                    <div className="flex items-start gap-2.5 min-w-0 flex-1">
                      {item.node.type === 'folder' ? (
                        <Folder className="h-4 w-4 text-amber-400 shrink-0 mt-0.5" />
                      ) : (
                        <span className={`rounded border px-1.5 py-0.5 text-[9px] font-extrabold uppercase shrink-0 mt-0.5 ${getMethodBadgeClass(item.node.method)}`}>
                          {item.node.method || 'GET'}
                        </span>
                      )}
                      <div className="min-w-0 space-y-1.5 flex-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <h5 className="font-bold text-xs text-slate-200">{item.node.name}</h5>
                          <span className="text-[9px] font-mono text-slate-500 uppercase px-1.5 py-0.5 rounded bg-slate-950 border border-slate-800">
                            {item.node.type}
                          </span>
                        </div>
                        
                        {/* Full Animated Hierarchy Path Indicator (No Truncation) */}
                        <div className="flex items-center gap-1.5 text-[11px] font-mono text-indigo-300 bg-slate-950/90 px-2.5 py-1 rounded-lg border border-indigo-900/60 w-fit max-w-full animate-in fade-in slide-in-from-left-2 duration-300 shadow-inner">
                          <Folder className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                          <span className="text-slate-400 font-semibold shrink-0">Hierarchy:</span>
