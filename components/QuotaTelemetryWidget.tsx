  ...
  return (
    <>
      <div 
        ...
        title="Click to view API Key Quota details and options"
      >
        <div className={`flex items-center space-x-2.5 rounded-xl border px-3 py-1.5 backdrop-blur-md shadow-sm transition-all hover:scale-[1.02] active:scale-95 ${
          isUserKey 
            ? 'bg-emerald-950/40 border-emerald-800/60 text-emerald-300 hover:border-emerald-500/60' 
            : quota.requestsUsed >= quota.maxDemoRequests
              ? 'bg-red-950/40 border-red-800/60 text-red-300 hover:border-red-500/60 animate-pulse'
              : 'bg-amber-950/40 border-amber-800/60 text-amber-300 hover:border-amber-500/60'
        }`}>
          ...
          <div className="flex items-center space-x-2">
            ...
            {!isUserKey && (
              <div className="w-16 bg-slate-900 rounded-full h-1.5 overflow-hidden border border-slate-800 hidden sm:block shrink-0">
                <div 
          ...
          <span className="flex items-center gap-1 font-mono text-[11px] text-slate-300 whitespace-nowrap">
          ...
          {quota.rateLimitStatus === 'quota_exceeded' && !isUserKey && (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-400 bg-amber-950 px-1.5 py-0.5 rounded border border-amber-700 whitespace-nowrap">
