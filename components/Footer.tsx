...
import { GitBranch, Mail, User, ExternalLink, ShieldCheck } from 'lucide-react';
import { useAdminStore } from '@/lib/admin-store';

export const Footer: React.FC = () => {
  const { showGithubLink, githubRepoUrl, anonymousMode } = useAdminStore();
  ...
  return (
    <footer className="w-full border-t border-slate-800/80 bg-slate-950/90 py-6 text-xs backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl flex-col md:flex-row items-center justify-between gap-4 px-4 sm:px-6">
        ...
        {/* Creator Identity / Anonymous White-Label */}
        <div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-3 text-slate-400 text-center sm:text-left">
          ...
          <span className="text-slate-400 text-xs">
            {anonymousMode ? 'Authorized Production System' : 'Architect & Lead Developer'}
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          {showGithubLink && (
            ...
            >
              ...
              <span>{anonymousMode ? 'Source Repository' : 'GitHub Repository'}</span>
        ...
        <div className="flex items-center space-x-2 text-[11px] text-slate-500">
          <span>{anonymousMode ? '© 2026 Enterprise Edition. All Rights Reserved.' : '© 2026 Ved Tripathi. All Rights Reserved.'}</span>
        </div>
